...
# Bare `text-foreground` is the L2 panel SURFACE token used as a text color —
# it renders text the same colour as its own background (invisible/near-invisible,
# and it flips wrong light↔dark since `foreground` denotes the surface, not a
# contrast label). Layout.astro line ~462 makes it explicit: shadcn primitives
# that say `text-foreground` for body text MUST become `text-font`.
#
# Deliberately NOT flagged:
#   • the `bg-font text-foreground` INVERSE button (design.md) — surface-coloured
#     text on a font-coloured fill is correct; exempted via the bg-font grep below.
#   • `text-{primary,secondary,destructive,sidebar}-foreground` — these are the
#     shadcn contrast-label idiom on brand fills (Button/Badge/Checkbox use them)
#     and resolve through globals.css's surviving `@theme inline`. Legit; not ours
#     to police here.
#   • `text-muted-foreground`/`text-card-foreground`/`text-popover-foreground`/
#     `text-accent-foreground` — Layout.astro aliases these to `--color-font`.
FOREGROUND_RE='text-foreground\b'
...
VIOLATIONS=$(
  {
    ...
    grep -nE "$FOREGROUND_RE" "$FILE" 2>/dev/null | grep -v 'bg-font'
  } | head -20
)
...
Replace banned utilities:
  ...
  text-foreground                →  text-font  (foreground is a surface color, not text)
                                    (on a brand fill use text-on-{primary|secondary|tertiary})
