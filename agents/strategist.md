...
Produces persona profile, positioning, and ICP definition for a campaign on receipt of a strategy signal.
...
- Extract persona id from the incoming signal; load persona dimensions from the substrate (identity, JTBD, inner world, problems, triggers, vehicle, awareness, vocabulary).
- Run Dunford positioning: name category-frame, alternatives, unique-attributes, unique-value, best-fit-customer.
- Score ICP fit against existing actors using fit-score; surface top three matching segments.
- Apply Schwartz awareness × sophistication map to recommend offer register and hook depth.
- Produce message-market-media match; reject any message with fit-score < 0.7.
- Emit `campaign:<id>:strategy-ready` with persona block, positioning block, ICP query, and awareness-level annotation.
- Mark the path on success; warn with weight 0.5 if persona confidence < 0.3.

## Output Format

`campaign:<id>:strategy-ready` carries structured data: persona dimensions snapshot, Dunford positioning object, ICP fit-score, recommended awareness level, and message-market-media fit-score.
