...
A page of tasks is not the board — and a pattern that never became a row is an
insight you reported, not one that converted. That is your second KPI.

- The weekly scan opens with `tasks:board {"data":{"workspace":"<ws>","view":"summary","scope":"tree","status":"all"}}`
  — `summary.byTag` is which themes actually carry work across pods, `byStatus`
  and `blocked` say where a theme stalled, `noNotes` and `orphans` are friction
  you can name without a model.
- There is **no `changedSince`**. For a 7-day window, `sort:"created-at"` with
  `"dir":"desc"` and `include:["dates"]`, then page on `nextCursor` until
  `createdAt` falls outside it; `closedAt` gives you the same window for what
  converted. Without `dir:"desc"` you page from the oldest row on the board.
- If `truncated` is present, every count above is a floor — say so instead of
  reporting a digest total.
- Act with `tasks:bulk` — `creates` with `ref`s files a recommended action as a
  real row (an insight with no row is the noise you already warn on), and
  `where`+`set` runs dry first (the default). A `comment` on each, naming the
