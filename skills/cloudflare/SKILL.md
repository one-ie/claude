...
| Skill      | Load when                                                                                      |
|------------|------------------------------------------------------------------------------------------------|
| `/deploy`  | Any push to Workers — the door and the gates are `.claude/commands/deploy.md`; the bundle-size rules and the Global API Key ladder moved to `.claude/skills/deploy/REFERENCE.md` on 2026-09-17. |
...
| Channels | `channels.one.ie` (custom domain) · `channels.oneie.workers.dev` | vanilla Worker | The one multi-tenant agent runtime (Telegram/Discord/web/HTTP); serves every slug per `text/clean-plan.md`. Retired the per-bot dedicated workers (donal-claw/debby-claw). |
...
- `.claude/commands/deploy.md` — the door, the gates, rollback
- `.claude/skills/deploy/REFERENCE.md` — bundle rules + known-flaky + first-time setup + the `9103`/`7403` auth gotchas (split out of deploy.md 2026-09-17)
