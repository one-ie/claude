...
## How it stays cheap — and why you never choose

You can run this for a typo *and* for a billing system because `/do` sizes the job before it starts. A typo gets one edit and one fast check; a new system gets the full spine above. You never pick a mode. You never set the spend. It reaches for the cheapest tool that can answer first — a text search before a small model, a small model before a large one — and a step that needs no AI call at all is the best kind of step. Most work is small, so most work is nearly free. And it speeds up as you trust it: a streak of strong results and it stops pausing for permission.
