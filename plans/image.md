...
**R2 + `img.one.ie` is explicitly out of scope** for this cycle. It needs a new bucket, a DNS record on a domain that currently only routes `app.one.ie`, and a public-read policy review. Tracked as follow-up below.
