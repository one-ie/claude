...
- [x] Implement register(): unit creation with kind and status
  ...
  exit: `persist.actor(id, kind, opts)` creates actor with aid, name, actor-type, status, timestamps. Works for all species.
...
- [x] TypeDB schema: unit status transitions
  ...
  phase: C1
  persona: dev
  exit: world.tql has actor-type enum, status tracking, created/updated timestamps. Functions: unit_status(aid), unit_classification(aid).
  tags: schema, lifecycle, P1
  done: world.tql has actor-type, unit_classification(), needs_evolution() functions
