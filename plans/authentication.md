...
## RP ID Rule — one credential per domain ecosystem

**WebAuthn credentials are bound to a Relying Party (RP) ID.** Different RP =
different identity to the browser. The rule:

> **RP ID is the registrable parent of the host.**

| Host | RP ID |
|------|-------|
| `one.ie`, `*.one.ie` | `one.ie` |
| `acme.com`, `*.acme.com` | `acme.com` |
| `localhost` | `localhost` |

One credential per domain tree. Today's `rpFromRequest()` uses the full host —
this is the multi-domain bug (Gap §9). The fix is two lines:

```ts
const ONE_SUFFIX = '.one.ie'
const rpId = host === 'one.ie' || host.endsWith(ONE_SUFFIX)
  ? 'one.ie'
  : host.replace(/^www\./, '')
```

Cross-domain (e.g. agency adds `acme.com` to a `one.ie` account) needs a second
credential — see Multi-Domain Credentials below.

---

## Authentication Per Lifecycle

One actor (slug) may carry one credential per RP. `owners_keys` is the set.

| Audience | RP at signup | When second credential needed | Recovery |
|----------|--------------|------------------------------|----------|
| `end_user` | — | never | — |
| `client` (independent) | `one.ie` | adds custom domain | BIP39 paper |
| `client` (inherited) | agency root (`acme.com`) | detaches from agency | BIP39 paper |
| `agency` | wherever they signed up | each new custom domain attached | BIP39 paper |
| `owner` | `one.ie` + `staff_role=1` | optional, per ops needs | paper + `mac.md` break-glass |

The slug — not the RP — is the access boundary. RP only governs which
credentials the browser will produce on this origin.

---

## Multi-Domain via Device-Link

When an actor adds a second domain (custom domain attach, agency upgrade,
client detach), they register a fresh credential at the new domain. No
private-key transfer.

```
existing domain (one.ie):  POST /api/link?action=issue { slug }
                            → returns { token } (HMAC, 10min, one-shot, bound to slug)

new domain (acme.com):     POST /api/link?action=redeem { token, registration }
                            → verify token, run verifyRegistrationResponse
                            → INSERT INTO owners_keys (slug, credential_id, pubkey, rp_id)
```

Schema extension — minimum needed for multi-RP lookup:
...
Verification picks the matching credential by `(slug, rpId)`. No match →
return a structured "no-credential" error so the UI can offer device-link
instead of a generic 401.

The primary credential currently on `owners` is migrated into `owners_keys`
as a row with `rp_id = 'one.ie'`. `owners` keeps only identity fields after
migration.

Recovery (BIP39) works at any reachable domain — the mnemonic re-derives the
right to enroll a new credential under the slug, with whichever RP is current.

---

## Invite-Based Provisioning

Agency wants client at `acme.com/u/startup1`. The credential must be created
at the agency's domain so its RP is right from the first ceremony.

```
agency:  POST /api/provision?action=create-invite { parent_slug, child_slug, email }
         → invite token (HMAC, 7-day TTL), URL: https://acme.com/u/{child}?invite={token}

client:  visits URL → registration ceremony at acme.com (RP = acme.com)
         POST /api/provision?action=redeem-invite { token, registration, slug }
         → INSERT owners (parent_slug = acme), INSERT owners_keys (rp_id = acme.com)
```

The agency cannot impersonate the client — the keypair is generated by the
client's authenticator. The agency can revoke by deleting the `owners` row
(cascade clears `owners_keys`).
...
| Host | Resolution |
|------|-----------|
| `one.ie`, `app.one.ie`, `localhost`, `127.0.0.1` | Primary — `workspace: undefined`, viewer forced to `'developer'` |
...
```
staffRole=true                         → 'owner'    (platform admin — Tony)
ownerSlug === workspaceSlug            → 'agency'   (owns the workspace they are viewing)
hasSession && ownerSlug ≠ workspaceSlug → 'client'   (authenticated, visiting another workspace)
otherwise                              → 'end_user' (anonymous visitor)
```
...
## 4-Tier Audience Model

| Tier | Who | How derived | Examples |
|------|-----|-------------|---------|
| `owner` | Platform admin (Tony) | `staffRole=true` | Accessing from `one.ie` with staff flag |
| `agency` | Workspace owner | `ownerSlug === workspaceSlug` | `alice.one.ie` while signed in as `alice` |
| `client` | Paying customer of an agency | Authenticated, viewing another's workspace | `alice.one.ie` while signed in as `bob` |
| `end_user` | Anonymous visitor | No session | Any workspace, no passkey |
...
Same table applies to `/u/[slug]/...` workspace routes (Skills → `[slug]/skills`, etc.).
...
### 9. `rpFromRequest` uses full host instead of registrable parent

A passkey registered at `alice.one.ie` does not validate at `one.ie/u/alice`,
and a `one.ie` passkey breaks the moment a custom domain is attached. **Fix:**
two-line change to return `'one.ie'` for `*.one.ie` and `host` (sans `www.`)
otherwise. See "RP ID Rule" above.

### 10. `owners_keys` cannot represent multi-RP credentials

No `rp_id` or `credential_id` column. Today's verifier uses `owners.pubkey` (a
single primary credential) — secondary devices already broken (Gap §3), and
multi-domain entirely impossible. **Fix:** migration adding two columns + an
index, plus `findCredential(slug, rpId)` to replace `getSlugOwner().pubkey` in
the verify path.

### 11. No device-link or invite-redeem endpoints

`/api/link` (issue + redeem) and `/api/provision?action=redeem-invite` are
required for the multi-domain and inherited-client lifecycles respectively.
Today neither exists. **Fix:** see roles-todo Wave 0g and Wave 0b.
