...
status: |
  Code shipped — W4 acceptance below all [x]. Live end-to-end receipts on
  the four surfaces (web · CLI · MCP · raw HTTP) still pending; that is
  the "close step" called out in the Receipt section.
post-ship-notes: |
  Subsequent work in the skills-todo cycle (C1–C16, May 2026) surfaced
  follow-ups that didn't exist when this plan landed:
  - SDK is anemic — only `skillsImport()`. Lifting parse/emit/eval into
    @oneie/sdk would let CLI/MCP/web/chat become thin wrappers (see the
    "one contract, four surfaces" architecture in skills.md).
  - `oneie skill eval` is still a stub. Becomes ~5 LOC once SDK has `evalSkill`.
  - `autoImportSkillCreator()` is defined but never called. New workspaces
    have empty catalogs.
  - `actions/skills.ts skill.eval` calls `runCase` twice per case (bug).
    Should be deleted in favour of routing through `/api/eval`.
  See `skills.md` § Open gaps for the prioritised list.
---
...
### Receipt capture script

`one.ie/web/scripts/skill-eval-receipt.sh` orchestrates the four-surface check:

1. **raw HTTP** — `curl POST /api/eval` with Bearer `<slug>:<key>` (C23 gate)
2. **CLI** — `oneie skill eval <fixture>` (C19)
3. **MCP** — `skill_eval` tool via Claude Desktop (C21) — interactive, manual capture
4. **web** — `/u/{slug}/skills/{name}#eval` → Run eval now (C5)

Requirements:
- `ONEIE_BASE_URL`, `ONEIE_SLUG`, `ONEIE_API_KEY` set (server-secret, not personal API key)
- A fixture skill with `evals:` in frontmatter — generate via `oneie skill new` (C19 template includes 3 example cases)

Run order:

```bash
ONEIE_SLUG=your-slug ONEIE_API_KEY=<server-secret> \
  ./one.ie/web/scripts/skill-eval-receipt.sh
```

Outputs the four pass_rates and the markdown row template to append below.

### Receipt log

| Date | Skill | Surface pass_rates | Agree (±0.05)? |
|------|-------|--------------------|----------------|
| _pending_ | _pending_ | http=? cli=? mcp=? web=? | _pending_ |

**Status (2026-05-24):** Script shipped; live execution requires a logged-in workspace with `SERVER_SECRET` access — pending owner run. The path is end-to-end functional: C18/C19/C20/C21/C23 closed the substrate, and the script encodes the contract; the only remaining work is one human running it once against a real workspace.
