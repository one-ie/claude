...
```typeql
insert $u isa actor, has aid "bob";
...
3. **Does bob have model + system-prompt?**
   ...
   curl http://localhost:4321/api/query -d '{
     "query": "match $u isa actor, has aid \"bob\", has model $m, has system-prompt $sp; select $m, $sp;"
   }'
   ```
...
# Check model is set
curl /api/query -d '{"query": "match $u isa actor, has aid \"bob\", has model $m; select $m;"}'
...
- [DSL.md](DSL.md) — Signal grammar and handlers
