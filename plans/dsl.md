...
```
alice             → direct actor, default skill
alice:review      → direct actor, named skill
world:review      → pheromone picks the best actor with "review" skill
world:review+P0   → pheromone picks actor matching both tags
all:review        → fan-out — every capable actor gets a copy
sub:news:crypto   → fan-out — every subscribed actor gets a copy
world             → strongest outgoing highway (rare)
```

Direct for commitment. `world:` for discovery. `all:` and `sub:` for fan-out.
See [signals.md](signals.md) for the full grammar, resolution flow, and cold-miss path.
...
Tasks use the 7-state machine. These are `task-status` values on `thing` entities:

| Status | Condition | Pheromone |
|--------|-----------|-----------|
| `open` | ready to pick | — |
| `blocked` | blocker task not `verified` | — |
| `picked` | agent has claimed it | — |
| `done` | result returned; W4 pending | mark(+depth) |
| `verified` | rubric ≥ 0.65 | mark(score×5) |
| `failed` | no result (not timeout) | warn(1) |
| `dissolved` | missing unit/capability | warn(0.5) |

**Wave → model routing** (every task's `task-wave` attr selects the executor):

| Wave | Model | Purpose |
|------|-------|---------|
| W1 | Haiku | Recon — fast parallel reads |
| W2 | Opus | Decide — reasoning and tradeoffs |
| W3 | Sonnet | Edit — code and doc synthesis |
| W4 | Sonnet | Verify — consistency + rubric |

`task-id` format: `{plan-slug}:{cycle}:{role}{index}` — e.g. `loop-close:1:e3`, `loop-close:3:e2.b` (variant B).
...
## Education Signal Conventions

Education-domain signals follow the base `{receiver, data}` contract with
three optional sub-keys in `data`. The signal layer never inspects these;
only the projection layer and analytics layer read them.

### Receiver format

```
education:<domain>:<event>   — projection layer subscription
learner:<actor-aid>          — direct learner signal
skill:<thing-tid>            — skill-scoped routing
```

### `data` sub-shapes for education signals

```typescript
data = {
  // tags: static facts about this signal — true at emission
  tags?: [{
    domain: "education",
    kind:   "attempt" | "mastery-change" | "prerequisite" | "eval",
    learner?: string,   // actor.aid
    item?:    string,   // thing.tid (thing-type='item')
    skill?:   string,   // thing.tid (thing-type='skill')
  }],

  // pheromones: environmental deposits — decaying, addressed to no one
  pheromones?: [{
    edge: { from: string, to: string },
    strength?:   number,   // mark() — path strengthens
    resistance?: number,   // warn() — path weakens
  }],

  // ontology: pre-computed ONE-dimension fragments (optional)
  // Projection layer enriches if absent — emitter never required to populate
  ontology?: {
    domain:  "education",
    kind:    "attempt" | "mastery-change" | "prerequisite" | "eval",
    learner?: string,
...
```json
{
  "receiver": "education:attempt:submitted",
  "data": {
    "tags": [{ "domain": "education", "kind": "attempt", "learner": "alice", "item": "q-001", "skill": "algebra" }],
    "pheromones": [{ "edge": { "from": "alice", "to": "algebra" }, "strength": 0.1 }],
    "weight": 0.1,
    "content": { "response": "x = 4", "correct": true, "elapsedMs": 12400 }
  }
}
```

**2. Mastery tier crosses threshold**
```json
{
  "receiver": "education:mastery:changed",
  "data": {
    "tags": [{ "domain": "education", "kind": "mastery-change", "learner": "alice", "skill": "algebra" }],
    "pheromones": [{ "edge": { "from": "alice", "to": "algebra" }, "strength": 0.8 }],
    "weight": 0.8,
    "content": { "from": "developing", "to": "proficient", "bktProb": 0.63 }
  }
}
```

**3. Prerequisite discovered**
```json
{
  "receiver": "education:prerequisite:discovered",
  "data": {
    "tags": [{ "domain": "education", "kind": "prerequisite" }],
    "content": { "required": "arithmetic", "dependent": "algebra", "confidence": 0.91 }
  }
}
```

### Weight vs score — the hard rule

`data.weight` is a pheromone deposit strength passed to `mark()` or `warn()`.
It MUST NOT carry an assessment score. If weight=0.8, it means "strongly
strengthen this path" — it says nothing about the learner's performance.

Assessment scores live in `data.content`. The projection layer reads content
to update `irt-theta` and `bkt-prob` on the relevant path.
