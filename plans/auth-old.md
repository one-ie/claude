...
The resolver `resolveUnitFromSession(request)` in `src/lib/api-auth.ts` is the single entry point. Both front doors return the same `AuthContext`. First call lazy-binds the actor (`actor-type: "human"`, deterministic Sui wallet from `SUI_SEED + aid`). Subsequent calls hit the 5-min cache.
