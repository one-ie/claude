...
const unit = world().add('hermes:researcher')
  .on('research_goal', async (data, emit, ctx) => {
        ...
        },
        {
          ...
          handler: async (problem) => {
            const peers = await readParsed(`
              match $p isa actor, has tag "researcher", has success-rate $sr;
              $sr >= 0.80;
    ...
    if (result.confidence < 0.50) {
      await write(`
        match $u isa actor, has aid "hermes:researcher";
...
Same rule for framework skins: a Langchain analyst has aid
