...
**Gate mode (`AUTH_GATE_MODE`):** `audit` (default, logs `security:gate:would-deny` signals but lets requests through) or `enforce` (returns 403 on role/scope denial). See [auth.md § Gate Matrix](apps/ants/docs/auth.md#gate-matrix) and [api-todo.md § Cycle 2](api-todo.md) for the rollout protocol.
...
- [auth.md](apps/ants/docs/auth.md) — Authentication deep-dive, wallet derivation, BetterAuth flows, gate matrix
