...
`world:` means "substrate, pick the best actor." `all:` fans out to every capable actor. `sub:` fans out to every opted-in subscriber. See [signals.md](signals.md) for the full five-mode grammar.
...
```typeql
# The actor entity (TypeDB) — called "actor" in the runtime layer
...
# Viewer's personal nicknames stored in KV, indexed by actor aid
...
| Field | What it means |
|-------|--------------|
| `actor-type` | `"human"`, `"agent"`, `"llm"`, `"system"` |
...
emits a signal with `data.tags` including the canonical tag below. See [lifecycle-one.md](lifecycle-one.md) for the full funnel.
...
## Auth — `Principal` & `requireAuth`

The unit of trust. Every route calls `requireAuth(request, action)` and receives a `Principal`. See [auth.md](auth.md) §4b, §4c.

| Term | Shape | Where |
|------|-------|-------|
| `Principal` | `{ user, slug, group, role, actions, bypass }` | `src/lib/principal.ts` |
| `requireAuth(request, action)` | `Promise<Principal>` — throws `AuthError(401\|403\|503, reason)` | `src/lib/api-auth.ts` |
| `AuthError` | `Error` with `status` + `reason: 'no_session' \| 'action_not_permitted' \| 'audit_write_failed' \| 'enforce_blocked'` | `src/lib/principal.ts` |
| `authErrorResponse(e)` | `Response` — serializes `AuthError` to JSON `{ error, reason }` with HTTP status | `src/lib/api-auth.ts` |
| `RoleAction` | 28-action union (frozen at cycle 8) — `add_unit, remove_unit, mark, warn, tune_sensitivity, read_highways, read_revenue, read_toxic, appoint_role, read_memory, delete_memory, discover, create_group, update_group, delete_group, invite_member, change_role, customize_vocabulary, edit_schema, mint_capability, add_attribute, view_onchain, chat_read, chat_write, vault_sync, mcp_register, verify_domain, grant_capability` | `src/lib/role-check.ts` |
| `MemberRole` | 6 roles (locked 2026-05-20) — `owner \| admin \| member \| viewer \| agent \| auditor` | `src/lib/role-check.ts` |
| `ownerBypass(auth, input)` | Returns `bypass \| enforce-block \| not-owner`. T2 stub returns `not-owner`; T12 fills with D1 audit write | `src/lib/role-check.ts` |
| `auditOwner(input)` | Writes `owner_audit` row in D1 BEFORE the bypassed action commits; T12 fills | `src/lib/role-check.ts` |
| `activeCapabilities(grantee, scope)` | Returns time-bounded `ActiveCapability[]` that `requireAuth` folds into the action set; T17 fills | `src/lib/capability-grant.ts` |

**Idiom — every route is one line of auth:**

```ts
export const POST: APIRoute = async ({ request }) => {
  const p = await requireAuth(request, 'chat_write')   // throws AuthError on 401/403
  // … business logic, naturally scoped to p.slug + p.group.gid
}
```

**Owner accountability.** When `role === 'owner'`, `bypass = true` and `ownerBypass()` is called before the action check. Every successful bypass writes an `owner_audit` row synchronously (T12); a failed audit write in enforce mode = 503, the action does NOT commit.

---

## Provisioning API — Slugs, Commits, and Recovery

Terms introduced in cycles C1–C7 for the slug-based site-mutation API
(`POST /api/provision`, `POST /api/commit`, `POST /api/recover`).
These concepts live at the boundary between WebAuthn identity and R2/D1 storage.

### Identity & Namespace

**`provision`** — `POST /api/provision`. One-time registration that creates a
new slug, inserts a D1 row, and stores the passkey credential. After provision
the owner can commit content to their namespace. Idempotent per credential;
calling it twice with the same passkey returns the existing slug.

**`slug`** — The owner's unique namespace identifier (e.g. `alice`). Used as:
R2 key prefix (`alice/index.md`, `alice/media/…`), URL path segment
(`one.ie/alice`), and D1 primary key. Chosen at provision time; immutable
thereafter.

**`domains table`** — D1 table (`domains`) mapping custom hostnames → slugs.
Astro middleware reads it on every request to rewrite `host → slug` so
`alice.one.ie` and `one.ie/alice` serve the same content. Schema:
`(hostname TEXT PRIMARY KEY, slug TEXT NOT NULL)`.

**`recovery token`** — HMAC-signed one-time KV entry with 900 s TTL, issued by
`/api/recover`. Lets an owner register a new passkey without the old device.
Consumed on first use; subsequent calls with the same token return 401. Key
format: `recovery:{slug}:{token}`.

### Write Safety

**`commit`** — `POST /api/commit`. HMAC-bound signed R2 write gated by a
WebAuthn assertion. Every mutation (page save, media upload, skill publish)
goes through this endpoint. Body carries `slug`, `path`, `content`,
`expectedSha`, and the WebAuthn `assertion`. Returns `{ sha }` on success,
409 on conflict, 401 on bad assertion.

**`challenge`** — HMAC-signed time-limited token (60 s TTL) that binds one
specific write operation. Generated by the server before each commit; the
client signs it with the passkey and returns it as part of the assertion.
Prevents replay attacks: a challenge is single-use and expires after 60 s.
KV key: `challenge:{slug}:{nonce}`.

**`assertion`** — The WebAuthn assertion produced by `navigator.credentials.get()`
on the client. Carries the signed challenge, authenticator data, and credential
ID. The commit endpoint verifies the assertion against the stored public key
before writing to R2. Failure → 401 (no write occurs).

**`conflict gate`** — The 409 response returned by `/api/commit` when the
`expectedSha` in the request does not match the current R2 object's SHA-256.
Guards against overwrite races in concurrent edits: the client must re-fetch,
merge, and resubmit. No data is written on a 409.

### Skill & Content

**`activation envelope`** — The pending-payment wrapper returned by a skill
endpoint before a paid skill runs. Contains the skill price and an x402 quote.
The client must settle the quote and resubmit; the skill executes only after
payment is confirmed. Shape: `{ status: "pending", price, quote }`.

**`import refresh`** — Background re-fetch of a remote `SKILL.md` triggered by
`POST /api/skill/refresh`. Bearer-authed; updates the cached skill definition
in R2 and D1 without requiring a full commit flow. Keeps skill definitions in
sync with upstream changes. See `web/src/pages/api/skill/refresh.ts`.

**`media`** — Binary assets (PNG / JPG / WebP / SVG) stored in R2 under
`<slug>/media/<sha>.<ext>`. Served with immutable cache headers; the SHA-keyed
path means the URL changes only when the content changes. Uploaded via
`/api/commit` with `path` set to the `media/` prefix.
...
## Education Domain Vocabulary

Extends the canonical vocabulary for the learning-as-a-process domain.
See `one/education-ontology.md` for the full spec; `one/education-schema.tql` for the TypeDB schema.

**The 6 dimensions, education lens:**

| Term | Definition | Entity.attribute | Source |
|------|-----------|-----------------|--------|
| `education-role` | Role of the actor in an education context | `actor.education-role` | education-schema.tql |
| `item-kind` | Type of learning object | `thing.item-kind` | education-schema.tql |
| `irt-a` | Item discrimination parameter (IRT 2PL) — 0.1–3.0 | `thing.irt-a` | education-schema.tql |
| `irt-b` | Item difficulty parameter (IRT 2PL) — −4.0–4.0 | `thing.irt-b` | education-schema.tql |
| `irt-theta` | Learner ability estimate per skill domain — −4.0–4.0 | `path.irt-theta` | education-schema.tql |
| `bkt-prob` | P(mastered) from Bayesian Knowledge Tracing — 0.0–1.0 | `path.bkt-prob` | education-schema.tql |
| `mastery-level` | Human-readable mastery tier derived from bkt-prob | `path.mastery-level` | education-schema.tql |
| `kst-state-id` | Knowledge Space Theory competence-state identifier | `thing.kst-state-id` | education-schema.tql |
| `edu-verb` | Caliper/xAPI-derived behavioural verb (see taxonomy) | `event.edu-verb` | education-schema.tql |
| `attempt-correct` | Whether an attempt was answered correctly | `event.attempt-correct`, `attempt.attempt-correct` | education-schema.tql |
| `attempt-elapsed-ms` | Time taken for an attempt in milliseconds | `event.attempt-elapsed-ms`, `attempt.attempt-elapsed-ms` | education-schema.tql |
| `prerequisite-of` | Relation: required skill → dependent skill | relation | education-schema.tql |
| `attempt` | Relation: learner × item × course-group | relation | education-schema.tql |

**Mastery tiers (bkt-prob thresholds):**

| Level | bkt-prob | Meaning |
|-------|----------|---------|
| `novice` | < 0.40 | No meaningful evidence of mastery |
| `developing` | 0.40–0.60 | Some evidence; not reliable |
| `proficient` | 0.60–0.80 | Reliable performance; not yet mastered |
| `mastered` | ≥ 0.80, n ≥ 20 | Mastery threshold; stake-eligible (Phase 4) |

**Education signal kinds (`data.tags.kind`):**

| Kind | When emitted |
|------|-------------|
| `attempt` | Learner submits a response to an item |
| `mastery-change` | bkt-prob crosses a tier boundary |
| `prerequisite` | A prerequisite-of relation is discovered or confirmed |
| `eval` | Eval harness judges an agent response (same mechanism as human attempt) |

**Group discriminators for education:**

| group-type | What it is |
|-----------|-----------|
| `course` | A bounded learning experience with a defined objective |
| `cohort` | A set of learners progressing together |
| `curriculum` | An ordered set of courses |

**Thing discriminators for education:**

| thing-type | What it is |
|-----------|-----------|
| `item` | A single assessment item (question, problem, reading, etc.) |
| `skill` | An ability domain; irt-theta and bkt-prob are per learner×skill path |
| `course` | A course-as-thing (when referenced as content, not as a group container) |

**Education dead names (never use):**

| Dead name | Use instead | Reason |
|-----------|-------------|--------|
| `student` | `learner` (education-role value) | Substrate uses actor; role discriminates |
| `grade` | `mastery-level` | Grade is an institution artifact; mastery is a measurement |
| `course-completion-percent` | `bkt-prob` | Percentage is synthetic; probability is the measured quantity |
...
- [signals-catalog.md](signals-catalog.md) — 100+ signals: implemented, beta, proposed
- [signal-integration.md](signal-integration.md) — **Implementation audit** — 25 wired, 15 design-ready, 60+ proposed
- [signal-wiring-pattern.md](signal-wiring-pattern.md) — How to add a new signal (step-by-step)
- [signal-wiring-roadmap.md](signal-wiring-roadmap.md) — Priorities for wiring remaining signals
