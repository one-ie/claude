...
> Narrative for humans: `text/do.md`. Per-stage template/skill/agent routing: `text/templates-plan.md`. **Historical** — `commands/do.md` is what `/do` executes now (incl. the three-manifest promise: `proof:` / `derives:` / `world:`); this file is kept for reference.
...
| **spec** | `test -f text/<slug>.md` | Opus + `text/template-spec-plan.md` (designs the gap list; pre-mortem + decisions live in the template), then `do-reconcile.sh` | opus · high |
| ↳ *CLARIFY* | spec has `## Clarifications` (FEATURE/SCHEMA) | ≤5 `AskUserQuestion`, written back into the spec | opus · high |
| **todo** | `test -f text/<slug>-todo.md` | `/create todo` + `text/template-todo.md` | sonnet · medium |
...
**One command, complexity-sized.** `/do <anything>` is the only thing a human types — an idea, a slug, or a `text/<slug>-todo.md` path. The tier sizes the work and the spine prunes itself: a typo is edited and verified inline (one cycle, no loop); a feature writes its promise, spec, todo, tests, and docs and then runs every cycle. The human never picks the mode, never types a flag, never runs a second command.
...
# NO baseline file is written here. Rung A5 (`fe8dfdace`) deleted the plan-start baseline
# artifact and every reader of it; this line was its last writer and produced a file nothing
# reads. The command it ran was the reason A5 killed it: `bunx tsc --noEmit` from the repo root,
# which has no tsconfig.json — 0.184s, exit 1, tsc's usage text, and `grep -c "error TS"` = 0.
# The verify output above IS the baseline. Quote it; do not re-add a writer.
```
...
**Templates**: `text/template-frame.md` (promise) · `text/template-spec-plan.md` (design + pre-mortem + decisions) · `text/template-todo.md` (plan + parallel budget + testing policy) · `text/agent-template-plan.md` (agent definition).
